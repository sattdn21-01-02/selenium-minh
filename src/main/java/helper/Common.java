...
public class Common {
    ...
    public static String getMessageValue(String key) {
        String result;
        try (InputStream inputStream = new FileInputStream(Common.getProjectPath() + Constant.TEST_RESOURCE_PATH +"message.properties")) {
    ...
    public static String getTestDataValue(String key) {
        String result;
        try (InputStream inputStream = new FileInputStream(Common.getProjectPath() + Constant.TEST_RESOURCE_PATH + "test-data.properties")) {
            Properties properties = new Properties();
            properties.load(inputStream);
            result = properties.getProperty(key);
        } catch (Exception e) {
    ...
    public static File readFile(String fileName) {
        File file = new File(Common.getProjectPath() + Constant.TEST_RESOURCE_PATH + fileName);
        if (file.exists()) {
            return file;
        }
        return null;
